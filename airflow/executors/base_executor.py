...
class BaseExecutor(LoggingMixin):
    ...
    def heartbeat(self):
        ...
        Stats.gauge('executor_task_queue_len', len(self.running), 1)
        Stats.gauge('executor_scheduler_queue_len', len(self.queued_tasks), 1)
        Stats.gauge('executor_open_slots_len', open_slots, 1)

        self.log.debug("statsd message is send!")
        ...
        for i in range(min((open_slots, len(self.queued_tasks)))):
            ...
            # scheduler_queue_time to task_queue_time
            Stats.gauge('scheduler_to_task_queue_time', (timezone.utcnow() - iqt).total_seconds(), 1)
